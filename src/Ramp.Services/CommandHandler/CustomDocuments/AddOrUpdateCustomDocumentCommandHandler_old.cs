...
namespace Ramp.Services.CommandHandler.CustomDocuments
{
    public class AddOrUpdateCustomDocumentCommandHandler : CommandHandlerBase<AddOrUpdateCustomDocumentCommand>
    {
        ...
        public override CommandResponse Execute(AddOrUpdateCustomDocumentCommand command)
        {
            ...
            if (Modelvalidation(command))
            {
                try
                {
                    ...
                    //run the forearch multiple times for each type of document selected
                    ...
                    if (command.TrainingMannual)
                    {
                        try
                        {
                            ...
                            //var collaboratorsTM = entryTM?.Collaborators ?? command.Collaborators?.ToList() ?? new List<StandardUser>();
                            //collaboratorsTM.RemoveAll(x => x == null); // command.Collaborators is populated with [null]
                            //if (standardUser != null && !collaboratorsTM.Contains(standardUser))
                            //{
                            //    collaboratorsTM.Add(standardUser);
                            //}
                            //DocumentCommandHandler.SyncCollaborators(entryTM, collaboratorsTM);
                    ...
                    if (command.CheckList)
                    {
                        try
                        {
                            ...
                            //var collaboratorsCL = entryCL?.Collaborators ?? command.Collaborators?.ToList() ?? new List<StandardUser>();
                            //collaborators.RemoveAll(x => x == null); // command.Collaborators is populated with [null]
                            //if (standardUser != null && !collaboratorsCL.Contains(standardUser))
                            //{
                            //    collaboratorsCL.Add(standardUser);
                            //}
                            //DocumentCommandHandler.SyncCollaborators(entryCL, collaboratorsCL);
                            ...
                            var coverPictureCL = _queryExecutor.Execute<SyncDocumentCoverPictureQuery, Upload>(new SyncDocumentCoverPictureQuery
                            {
                                ExistingUploadId = entryCL.CoverPicture?.Id,
                                //ModelId = command.CoverPicture//?.Id
                            });
                            entryCL.CoverPicture = coverPicture;
                            ...
                            //entryCL.NotificationInteval = command.NotificationInteval;
                            //entryCL.NotificationIntevalDaysBeforeExpiry = command.NotificationIntevalDaysBeforeExpiry;
                            //entryCL.CheckListExpiresNumberDaysFromAssignment = command.CheckListExpiresNumberDaysFromAssignment;
                            //entryCL.IsGlobalAccessed = command.IsGlobalAccessed;
                            //entryCL.IsChecklistTracked = command.IsChecklistTracked;
                    ...
                    if (command.Test)
                    {
                        ...
                        try
                        {
                            ...
                            entry.CoverPicture = coverPicture;
                            //var certificateTest = _queryExecutor.Execute<FetchByIdQuery, Certificate>(new FetchByIdQuery { Id = command.Certificate?.Id });
                            //entryTest.Certificate = certificateTest;
                            //entryTest.CertificateId = certificateTest?.Id;
                    ...
                    if (command.Policy)
                    {
                        try
                        {
                            ...
                            //var collaboratorsPolicy = entryPolicy?.Collaborators ?? command.Collaborators?.ToList() ?? new List<StandardUser>();
                            //collaborators.RemoveAll(x => x == null); // command.Collaborators is populated with [null]
                            //if (standardUser != null && !collaborators.Contains(standardUser))
                            //{
                            //    collaboratorsPolicy.Add(standardUser);
                            //}
                            //DocumentCommandHandler.SyncCollaborators(entryPolicy, collaboratorsPolicy);

                            //var categoryPolicy = _queryExecutor.Execute<FetchByIdQuery, DocumentCategory>(new FetchByIdQuery { Id = command.Category?.Id });
                            //if (category != null)
                            //    entryPolicy.CategoryId = category.Id;
                            //var coverPicturePolicy = _queryExecutor.Execute<SyncDocumentCoverPictureQuery, Upload>(new SyncDocumentCoverPictureQuery
                            //{
                            //    ExistingUploadId = entry.CoverPicture?.Id,
                            //    //ModelId = command.CoverPicture?.Id
                            //});
                            //entryPolicy.CoverPicture = coverPicture;

                            //entryPolicy.Description = command.Description;
                            //entryPolicy.DocumentStatus = command.DocumentStatus;
                            //entryPolicy.LastEditDate = command.CreatedOn;
                            //entryPolicy.LastEditedBy = userId;
                            //entryPolicy.Points = command.Points;
                            //entryPolicy.PreviewMode = command.PreviewMode;
                            //entryPolicy.Printable = command.Printable;
        ...
        #region policy
        private void SyncContentPolicy(IEnumerable<PolicyContentBoxModel> command, Policy entry)
        {
            var existingIds = entry.ContentBoxes.Where(c => !c.Deleted).Select(c => c.Id.ToString()).ToArray();
            var commandIds = command.Where(c => !c.Deleted).Select(x => x.Id).ToArray();
            var removedChapters = existingIds.Where(id => !commandIds.Contains(id));
