...
namespace Ramp.Services.CommandHandler {
	public class MemoCommandHandler : ICommandHandlerBase<CreateOrUpdateMemoCommand>,
                                      IValidator<CreateOrUpdateMemoCommand>,
										ICommandHandlerBase<RestoreMemoCommand>,
									  ICommandHandlerAndValidator<DeleteByIdCommand<Memo>>
    {
		...
		public CommandResponse Execute(RestoreMemoCommand command) {
			...
			var restoreDoc = new RestoreTrack {
				...
				DocumentType = DocumentType.Memo.ToString(),
		...
		public CommandResponse Execute(CreateOrUpdateMemoCommand command)
        {
            var userId = (Thread.CurrentPrincipal as ClaimsPrincipal)?.Claims.FirstOrDefault(claim => claim.Type == ClaimTypes.NameIdentifier)?.Value;
            var standardUser = _standardUserRepository.Find(userId.ConvertToGuid());
        ...
        private void SyncContent(CreateOrUpdateMemoCommand command, Memo entry)
        {
		...
		private IEnumerable<MemoContentBox> CreateOrUpdateContent(Memo entry, params AcrobatFieldContentBoxModel[] models)
        {
        ...
        public IEnumerable<IValidationResult> Validate(CreateOrUpdateMemoCommand command)
        {
        ...
        public IEnumerable<IValidationResult> Validate(DeleteByIdCommand<Memo> argument)
        {
            if (_repository.Find(argument.Id) == null)
                yield return new ValidationResult("Id", $"Cannot find Memo with id : {argument.Id}");
        }

        public CommandResponse Execute(DeleteByIdCommand<Memo> command)
        {
            var e = _repository.Find(command.Id);
            e.Deleted = true;
